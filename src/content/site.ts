...
export const insights: Insight[] = [
  {
    slug: 'the-old-system-has-no-api-here-s-what-actually-fixes-that',
    title: 'The Old System Has No API. Here\'s What Actually Fixes That.',
    date: 'Sep 25, 2026',
    readingTime: '7 min read',
    category: 'Engineering',
    excerpt: 'No API on your old system? Here\'s what a real integration project includes—two-way sync, retry logic, record-level alerts, reconciliation—plus honest pricing and when off-the-shelf beats custom.',
    body: [
      { type: 'h2', text: 'The hour your team keeps losing' },
      { type: 'p', text: 'Somewhere in your company, someone opens two windows side by side. One is the old system: the dispatch platform, the claims database, the ERP nobody quite remembers installing. The other is the new one, or the spreadsheet, or the portal a customer expects to see updated. They read a number off one screen and type it into the other. All day. Every day. And everyone calls it part of the job.' },
      { type: 'p', text: 'In logistics it\'s freight status. In healthcare it\'s a patient record that lives in two places and occasionally disagrees with itself. In field services it\'s a job marked done in one system and left open in another until a customer calls asking why. It\'s rarely dramatic. It\'s just the hour your team keeps losing, every day, to a gap between two systems that were never built to talk to each other.' },
      { type: 'p', text: 'Sales decks call that gap "no API." That\'s not one thing.' },
      { type: 'h2', text: 'What \'no API\' actually means' },
      { type: 'p', text: 'Sometimes the vendor is gone. The company that built the system folded or got bought out years ago, and nobody\'s coming to add an API because nobody\'s home.' },
      { type: 'p', text: 'Sometimes the software predates the idea. It was built before webhooks and REST endpoints existed, so it has no concept of either. Not a design flaw at the time. Just old.' },
      { type: 'p', text: 'And sometimes the API exists and the vendor charges a fortune for it: rate-limited, license-gated, sold as a tier above what you\'re already paying for. We\'re not going to guess at why a vendor prices it that way. We just tell you what it\'ll cost to work around, or through, it.' },
      { type: 'p', text: 'Each of these needs a different fix. Knowing which one you\'re facing changes the conversation before it starts.' },
      { type: 'h2', text: 'Before you build anything, ask this' },
      { type: 'p', text: 'Is there an off-the-shelf tool that already does this for a tenth of the price? Two common CRMs with a documented connector between them. A payment processor with a standard adapter. An accounting package everyone already integrates with. If the tool exists and it\'s built for your exact pairing, buying it beats building it, every time.' },
      { type: 'p', text: 'We say so if there is. We turn down projects a specialist would do better, or that an existing product already solves for a tenth of the price. You\'ll hear that in the first call, not the third, and not after a discovery week has already been billed.' },
      { type: 'p', text: 'Custom integration work earns its cost when the systems involved are old enough or specific enough that nobody built a connector for them: a legacy database with no vendor left to ask, a scheduling system that only exports via file drop, a pairing too uncommon for a vendor to have bothered with. That\'s most of what we actually get called about.' },
      { type: 'h2', text: 'What a real integration project actually includes' },
      { type: 'p', text: 'A script that pulls a nightly export into a new table is not an integration. It\'s a one-way trickle. It works fine right up until someone edits a record on the new side and the old side never finds out.' },
      { type: 'p', text: 'A real integration project runs two-way sync, so changes on either side reach the other. It includes retry logic that can tell the difference between "this record failed" and "this system is temporarily unreachable," because those need different responses. It includes reconciliation, so both sides can be checked against each other on an ongoing basis rather than assumed to match because the sync ran. And it includes failure alerts that name the specific record: not "sync error," not a line buried in a log file, but a message that says which invoice, which patient record, which shipment failed, and why.' },
      { type: 'p', text: 'Skip any one of these and you haven\'t built an integration. You\'ve built a faster way to lose track of the same data.' },
      { type: 'h2', text: 'One record failing shouldn\'t take the whole sync down' },
      { type: 'p', text: 'We see this constantly in inherited systems: one malformed record, a date field with a typo, a duplicated customer ID, halts an entire batch sync. Ten thousand records were supposed to move overnight. None did, because record 4,417 had a field the receiving system didn\'t like.' },
      { type: 'p', text: 'That shouldn\'t happen. The other 9,999 should go through. The one that failed should get flagged and surfaced with enough detail that someone can fix it in minutes. "Sync failed, code 500" tells your team nothing. "Invoice #88213 failed: customer ID mismatch" tells them exactly where to look.' },
      { type: 'h2', text: 'Reconciliation doesn\'t end at go-live' },
      { type: 'p', text: 'Most write-ups treat reconciliation as a migration step: move the data, check it matches, move on. Fine as far as it goes. But the moment two systems are syncing both ways, drift becomes a permanent risk, not a one-time one.' },
      { type: 'p', text: 'Six months after launch, a bad deploy or an untested edge case can cause the two systems to quietly disagree. Nobody notices until a customer calls about an invoice that doesn\'t match what your team sees.' },
      { type: 'p', text: 'That\'s why we treat reconciliation as ongoing, not a checkpoint. Both sides get checked against each other on a schedule, with a clear rule for which system wins when they disagree. It\'s a feature of the project, running for as long as both systems are live.' },
      { type: 'h2', text: 'Once two systems are talking, stop stitching spreadsheets' },
      { type: 'p', text: 'You know the spreadsheet everyone secretly runs the business on. Someone rebuilds it every Friday from exports out of two or three systems, formulas nobody fully trusts, and it somehow ends up in a board deck anyway.' },
      { type: 'p', text: 'Once systems talk to each other reliably, that spreadsheet\'s job can move into one warehouse for cross-system reporting, a single place finance can query without knowing which system holds which piece of the truth. It\'s usually not why people fix the integration problem. It\'s why they\'re glad they did, three months later, when someone asks for a number and gets it in a minute instead of a day.' },
      { type: 'h2', text: 'What it actually costs and how long it takes' },
      { type: 'p', text: 'Integrations & data work typically runs 4 to 10 weeks and starts from $22k. That range is wide on purpose. A two-system sync where one side has a modern API is a very different job from a legacy database with no documentation and three overlapping coding schemes from past migrations. It\'s priced after scoping the actual systems, not before.' },
      { type: 'p', text: 'That scoping is what a discovery week is for: flat $4,800, credited back in full if you continue, ending in a written scope, an architecture sketch, and a fixed estimate you can hold us to.' },
      { type: 'h2', text: 'Why this is usually the first project worth doing' },
      { type: 'p', text: 'If you\'re weighing a bigger build, a full CRM or a custom web app, integration work is usually the smarter place to start. Partly because it\'s cheaper: 4 to 10 weeks and from $22k, against 10 to 20 weeks and from $45k for a custom web app, or 8 to 18 weeks and from $38k for a CRM.' },
      { type: 'p', text: 'Mostly because it\'s lower risk. A smaller, well-scoped project shows you how we scope, how we communicate, and whether the estimate holds, before you commit to something bigger. If it goes well, you\'ve fixed a real daily cost and you know what the next phase looks like. If it doesn\'t feel right, you\'ve lost a few weeks, not a year. See pricing for the full breakdown.' },
      { type: 'h2', text: 'For the regulated-adjacent: NDA and security review, honestly stated' },
      { type: 'p', text: 'If you\'re in healthcare, fintech, insurance, or logistics, this question comes up before pricing does: can we trust you with this data. We work under NDA by default, and we complete vendor security questionnaires as part of onboarding, because that\'s standard for getting approved by procurement in these industries.' },
  ...
  {
    slug: 'before-you-hire-a-web-app-development-company-answer-this-on',
    title: 'Before You Hire a Web App Development Company, Answer This One Question',
    date: 'Sep 23, 2026',
    readingTime: '6 min read',
    category: 'Strategy',
    excerpt: 'What actually separates a web application from a website, the signs you need one, and how to vet a custom web application development company before you sign anything.',
    body: [
      { type: 'p', text: 'Ask the vendor which one they think you\'re buying before you sign anything. If they don\'t ask you back, that\'s information too.' },
      { type: 'h2', text: 'A website and a web app are not the same purchase' },
      ...
      { type: 'p', text: 'Most businesses we talk to already have a web app. They just built it in a spreadsheet. It lives on someone\'s laptop with a version history that\'s really a folder full of files named "final_v3_ACTUAL," and half the company has learned to work around it instead of asking anyone to fix it. That spreadsheet everyone secretly runs the business on is a web app in disguise. It just doesn\'t have logins, doesn\'t track who changed what, and breaks the moment two people touch it at once.' },
      { type: 'p', text: 'Knowing which one you\'re buying changes who you should hire, what a fair price looks like, and what "done" even means.' },
      { type: 'h2', text: 'Why hiring the wrong agency burns the budget' },
      { type: 'p', text: 'A website agency builds for content and conversion. That\'s the discipline they\'ve hired for, priced for, and staffed for. Ask them to bolt on a customer portal with logins and permissions and you\'ll get exactly that: bolted on.' },
      { type: 'p', text: 'Role-based access, audit trails, and reporting a finance team can trust are a different discipline entirely. These aren\'t a plugin you add at the end. They\'re decisions about who can approve a discount, who can see a margin, who gets notified when a record changes, and what happens when two people edit the same job at once. Get this wrong and you don\'t get a failed project. You get a pretty front end sitting on top of the same broken process, with a bigger invoice attached.' },
      { type: 'h2', text: 'The signs you actually need a web app' },
      { type: 'p', text: 'The spreadsheet everyone secretly runs the business on has outgrown what a spreadsheet can safely do: formulas nobody wants to touch, a "master" version, someone who reconciles it by hand every Friday. Approvals happen over email, which means an approval is really just a sentence in someone\'s inbox that nobody can search six months later. And different people need different levels of access to the same data — your dispatcher shouldn\'t see payroll, your finance lead shouldn\'t have to ask a favor to see a job\'s real margin, but right now everyone\'s looking at the same file because nobody\'s built the version that doesn\'t require that.' },
      { type: 'p', text: 'If two of those are true, you\'re not shopping for a nicer website. You\'re shopping for a web app.' },
      { type: 'h2', text: 'What this actually looks like when we build it' },
      { type: 'p', text: 'Compare this to whatever a vendor is pitching you. A custom web application from us means role-based access, audit trails and approvals built in from the start, not added later. It means reporting built for finance, numbers they can trust without a side conversation to confirm them. It means migrating your existing data instead of asking your team to re-enter three years of history by hand. And it means training and documentation at the end, so the system doesn\'t live only in the heads of the people who built it.' },
      { type: 'p', text: 'We build it on React, Node and Postgres. Boring technology, on purpose. Novelty is a cost paid every year after launch, and your team is the one who pays it.' },
      { type: 'p', text: 'A typical build runs 10 to 20 weeks and starts from $45k. That\'s a range, not a promise, because your version of "approvals" might be one button or a four-stage sign-off with escalation rules. That difference is exactly what discovery is for.' },
      { type: 'h2', text: 'Sometimes the honest answer is: don\'t hire us' },
      { type: 'p', text: 'We turn down projects. Sometimes a specialist would do the job better than we would — a niche compliance tool, a vertical product where someone else has spent years solving exactly your problem. Sometimes an off-the-shelf product already solves it for a tenth of the price, and building custom would just be an expensive way to reinvent something you could buy this afternoon.' },
      { type: 'p', text: 'You\'ll hear that in the first call, not the third. A project we shouldn\'t have taken costs both of us more than it\'s worth.' },
      { type: 'h2', text: 'How the number gets fixed: discovery week' },
      { type: 'p', text: 'Every published price on our site starts with the word "from," because nobody can price a system they haven\'t scoped.' },
      ...
      { type: 'p', text: 'Do the scoping before you quote, not after. That\'s the whole trick to an estimate that holds.' },
      { type: 'h2', text: 'Who owns the code — the actual answer' },
      { type: 'p', text: 'You own the code outright from day one. The repos live in your own organization\'s account. The infrastructure runs in your own cloud account. If you want to walk into your own AWS console and see exactly what\'s running and what it costs, you can, from the start.' },
      { type: 'h2', text: 'If it ends, here\'s what you\'re left holding' },
      { type: 'p', text: 'If the relationship ends, you\'re not left holding a system nobody understands. We do a full handover: an architecture walkthrough with whoever inherits the system, runbooks that explain how it actually operates day to day, and two weeks of support after the handover to catch what the walkthrough missed.' },
      { type: 'p', text: 'Ask any vendor you\'re considering to describe theirs in the same amount of detail.' },
      { type: 'h2', text: 'A short checklist for vetting any web app company' },
      { type: 'p', text: 'Use this whether you call us or not.' },
      { type: 'p', text: 'Ask who owns the code, the actual repo, not a vague "you\'ll have access." Ask how the estimate was built, and specifically whether it came before or after anyone looked at your actual process. Ask whether roles and audit trails are covered as a first-class part of the build, not quoted separately later. Ask what handover looks like if things end: what documents, what walkthrough, and for how long.' },
      { type: 'p', text: 'If a vendor gives you a firm number over email before asking a single question about your data or your process, that number isn\'t real yet. The custom web application you actually need — and the ongoing cloud and care it takes to keep it running once it\'s live — deserves better than a guess.' },
    ],
  },
  {
    slug: 'your-old-system-doesn-t-have-an-api-that-s-not-the-problem-y',
    title: 'Your Old System Doesn\'t Have an API. That\'s Not the Problem You Think It Is.',
    date: 'Sep 21, 2026',
    readingTime: '6 min read',
    category: 'Engineering',
    excerpt: 'Legacy software without an API is the norm, not the exception. Here\'s the honest way to connect it: real access methods, real costs, and when not to integrate at all.',
    body: [
      { type: 'p', text: 'Nobody tells us their dispatch board has an API. We\'d be surprised if it did. Legacy systems without APIs aren\'t the exception in logistics, healthcare, field services and manufacturing — they\'re the norm. The dispatch board, the quoting tool, the intake form, the nightly reconciliation — this is the software that breaks quietly, and it\'s usually the software nobody thought to make talk to anything else, because ten or fifteen years ago nothing else needed to hear from it.' },
      { type: 'p', text: 'So if you\'re staring at a system with no API and wondering if you\'re the problem, you\'re not. You\'re just late to a conversation everyone in your industry is having.' },
      { type: 'h2', text: 'What "no API" actually means, and how each workaround breaks' },
      { type: 'p', text: 'There\'s a real menu here, and it\'s worth knowing all four items on it, because each one fails in a specific and predictable way.' },
      { type: 'p', text: 'Screen scraping and browser automation read the system the way a person would — by looking at the screen. It works right up until the vendor changes a screen layout, at which point it breaks completely and without warning. You find out when the numbers stop moving.' },
      { type: 'p', text: 'File drops move data in batches — an export at midnight, a CSV every hour. This is not real-time and never will be. It\'s fine for a lot of jobs. But a malformed file fails silently: nobody\'s watching a batch job the way they watch a live connection, so a bad file from Tuesday can sit there for a week before anyone notices the numbers are off.' },
      { type: 'p', text: 'Database triggers, writing or reading directly against the vendor\'s own database, can be the most reliable path when you control the infrastructure. It can also be the most dangerous. Many vendor license agreements explicitly prohibit direct database access, and even where they don\'t, writing data back into a system\'s own database can corrupt application-layer state that the vendor\'s software depends on to function. You\'re not just moving data. You\'re editing the assumptions the software makes about itself.' },
      { type: 'p', text: 'RPA band-aids script against a specific user interface — click here, read this field, type that. They work exactly as well as screen scraping and fail for the same reason: the day that UI changes, the script doesn\'t know, and it keeps clicking on nothing.' },
      { type: 'p', text: 'All four of these show up hardest on legacy systems, because legacy systems were never built with integration in mind. They were built to run one business process, in one place, and nobody on the original team was thinking about what would need to read from it in 2026.' },
      { type: 'h2', text: 'The cost of the duct tape you already have' },
      { type: 'p', text: 'Here\'s the part that doesn\'t show up on an invoice anywhere, which is exactly why it survives so long. Someone on your team is re-keying data by hand between two systems every day — copying a job number from the CRM into the billing system, or a delivery confirmation from the driver\'s app into the spreadsheet finance actually trusts. That\'s the hour your team keeps losing to work that should be automatic, every day, forever, until someone decides it isn\'t normal anymore.' },
      { type: 'p', text: 'And somewhere in the building, there\'s a spreadsheet — the spreadsheet everyone secretly runs the business on — that has quietly become the reconciliation layer nobody officially built and nobody fully trusts. It\'s not on any architecture diagram. It\'s just where the truth ends up living because nothing else agrees with itself.' },
      { type: 'h2', text: 'The honest fix: sync that tells you when it\'s wrong' },
      { type: 'p', text: 'A properly built integration doesn\'t eliminate the problems above. It makes them visible and recoverable, which is the whole difference.' },
      { type: 'p', text: 'Two-way sync with retry logic means a dropped connection doesn\'t mean a dropped record — the system notices the failed attempt and tries again, instead of quietly losing the update. Reconciliation runs on a schedule, not just at the moment two systems first connect, checking for drift even when everything looks fine on the surface. And when something does fail, the alert names the specific record that failed — not a generic "sync failed" email that tells someone to go check everything. Nobody has time to check everything.' },
      { type: 'p', text: 'This is what our integrations & data work is actually built to do — APIs where they exist, ETL and webhooks where they don\'t, with retry and reconciliation built into the design instead of bolted on after the first outage. It\'s not more exciting than duct tape. It\'s supposed to be boring. That\'s the point.' },
      { type: 'h2', text: 'Not everything needs to be integrated' },
      { type: 'p', text: 'Here\'s the part most vendors won\'t say, because it shrinks the invoice: the instinct to wire every tool to every other tool creates more failure points, not fewer. Ten systems all syncing to each other is forty-five possible connections, each with its own way of breaking at 2am.' },
      { type: 'p', text: 'The better call, more often than people expect, is picking one system as the source of truth and syncing everything else to it — one direction, one owner, one place to look when the number looks wrong. It\'s less impressive on a slide. It\'s also cheaper to run and easier to debug at 2am.' },
      { type: 'p', text: 'We choose boring technology on purpose, and the same logic applies to the integration layer itself: novelty there is a cost paid every year after launch, by you, not by us. We\'ll say this in the first call, not the third, because a simpler fix is a simpler fix, and there\'s no version of this business where we make more money by pretending otherwise.' },
      { type: 'h2', text: 'The end state worth building toward: one warehouse, not a tangle of point-to-point syncs' },
      { type: 'p', text: 'If you keep adding systems the way most companies do — one integration at a time, whenever the pain gets bad enough — you eventually end up with a dozen point-to-point syncs, each with its own failure mode, and nobody who can draw the whole picture on a whiteboard.' },
      { type: 'p', text: 'The better end state is one warehouse for cross-system reporting: everything feeds one place, and reporting reads from that one place instead of five systems that each claim to have the real number. This matters most for finance and reporting teams, who don\'t need five sources. They need one number they can defend in a meeting.' },
      { type: 'h2', text: 'What this costs and how long it takes' },
      { type: 'p', text: 'Integrations & data work starts at $22k and typically runs four to ten weeks, depending on how many systems are involved and how much history needs to move, not just flow forward.' },
      ...
      { type: 'p', text: 'If a connector already exists that does the sync you need, the right answer is to say so, not sell you a custom build on top of it. We turn down projects a specialist would do better, or that an existing off-the-shelf product already solves for a tenth of the price. That\'s not modesty. It\'s just true, and you\'ll find out either way — better to hear it from us on the first call than discover it after the invoice.' },
    ],
  },
  {
    slug: 'what-a-web-development-company-actually-does-and-when-you-do',
    title: 'What a Web Development Company Actually Does (And When You Don\'t Need One)',
    date: 'Sep 18, 2026',
    readingTime: '5 min read',
    category: 'Engineering',
    excerpt: 'What a web development company actually does, where the work stops, and what it costs — real prices, real timelines, no case studies, no jargon.',
    body: [
      { type: 'p', text: 'Two people type "web development company" into Google in the same hour. One wants a new marketing site with a nicer hero image. The other is staring at a spreadsheet with fourteen tabs, three macros nobody remembers writing, and a dispatch board held together by copy-paste. They\'re done. This piece is for the second person.' },
      { type: 'p', text: 'If you\'re trying to replace the spreadsheet everyone secretly runs the business on — the quoting tool, the intake form, the nightly reconciliation one person does by hand — you\'re not shopping for a website agency. You\'re shopping for something closer to a software vendor. The work looks different depending on your industry. The buying decision doesn\'t.' },
      { type: 'h2', text: 'The actual range of work under that name' },
      { type: 'p', text: 'For this kind of buyer, "web development company" usually breaks into six real categories. A decent vendor tells you which one you need instead of selling you all six.' },
      { type: 'p', text: 'Custom web applications — portals, dashboards, workflow tools finance and ops teams live in all day. React, Node, Postgres. Typically 10–20 weeks, from $45k. (details)' },
      { type: 'p', text: 'Mobile apps — iOS and Android for field crews, technicians and customers, built offline-first so a bad signal doesn\'t cost a day\'s work. Swift, Kotlin, React Native. Typically 12–22 weeks, from $60k. (details)' },
      { type: 'p', text: 'CRMs and internal tools — quoting, dispatch, scheduling, invoicing into your accounting package. Typically 8–18 weeks, from $38k. (details)' },
      { type: 'p', text: 'AI and automation — document and email extraction, search over your own documents, with accuracy testing and a human review step built in, not assumed. Typically 6–14 weeks, from $32k. (details)' },
      { type: 'p', text: 'Integrations and data — two-way sync with retry and reconciliation, including legacy systems that were never built with an API. Typically 4–10 weeks, from $22k. (details)' },
      { type: 'p', text: 'Cloud, DevOps and care — infrastructure as code, running in your own AWS account. Ongoing, from $4.5k a month. (details)' },
      { type: 'h2', text: 'Where the work stops' },
      { type: 'p', text: 'We turn work away. We say so on the first call, not the third. Two reasons cover almost all of it. First, a specialist would do it better, and we\'d rather tell you that than take the fee. Second, an off-the-shelf product already solves the problem for a tenth of the price. If your quoting process is unusual enough to need a custom build, that\'s real work. If it\'s a standard workflow an existing product already handles, paying us for it is a way to overpay for something you could subscribe to this afternoon.' },
      { type: 'h2', text: 'How the engagement actually starts' },
      { type: 'p', text: 'We don\'t quote off a sales call. We run a discovery week — flat $4,800, credited back in full if you continue with us — and it ends in three things: a written scope, an architecture sketch, and a fixed estimate.' },
      { type: 'p', text: 'Most vendors give you a number before they understand your data, your integrations, or the one legacy system nobody wants to touch. Then they spend the project renegotiating it. We think estimates should come after discovery, not before. It\'s the only way the number holds.' },
      { type: 'h2', text: 'What happens if the estimate turns out to be wrong' },
      { type: 'p', text: 'The prices above are starting points, not final quotes. Discovery is what locks the real number for your project. Once it\'s locked, scope changes get priced before the work starts, not slipped into an invoice after.' },
      { type: 'p', text: 'A fixed-scope project runs from $28k per phase, billed 30% at kickoff and the rest on phase acceptance, and includes 30 days of post-launch fixes. If something changes mid-phase, you get a price for the change before we touch anything.' },
      { type: 'h2', text: 'Who owns the code' },
      { type: 'p', text: 'You own the code, from day one. The repos live in your organization\'s account. The infrastructure runs in your own cloud account. That\'s a deliberate choice against vendor lock-in. If we disappeared tomorrow, you\'d still have the keys.' },
      { type: 'h2', text: 'Who\'s actually on the project' },
      { type: 'p', text: 'We work alongside in-house teams — that\'s normal. What determines whether a project goes well isn\'t headcount, it\'s continuity: a named lead stays with your project from the scoping call through launch. No handoff to whoever\'s free that sprint.' },
      { type: 'p', text: 'For work that runs longer or needs sustained capacity alongside your team, there\'s a dedicated squad option — one designer, two engineers, one lead — from $18k a month, billed monthly in advance, on rolling three-month terms.' },
      { type: 'h2', text: 'Boring technology, on purpose' },
      { type: 'p', text: 'React, Node, Postgres for web apps. Swift, Kotlin, React Native for mobile. AWS and CI/CD underneath. None of that is exciting. Novelty is a cost paid every year after launch, by the client, not the vendor. We\'d rather ship something a competent engineer can maintain in five years than something that looked clever in the demo.' },
      { type: 'h2', text: 'NDAs and security review' },
      { type: 'p', text: 'We sign NDAs by default and complete vendor security questionnaires as part of onboarding. We\'re not going to claim a formal certification we don\'t hold. What we will do is answer the questionnaire honestly and put the access controls in place the review actually asks for.' },
      { type: 'h2', text: 'How the bill actually works' },
      { type: 'p', text: 'Three shapes. Fixed-scope: 30% at kickoff, the rest on phase acceptance. Dedicated squad: from $18k a month, billed monthly in advance, rolling three-month terms. Care and improvement: from $4.5k a month, cancel with 30 days\' notice. Everything runs Net 14.' },
      { type: 'p', text: 'We don\'t take equity and we don\'t defer payment. It muddies the relationship, and it has never once made the software better. Cash, milestones, clear scope.' },
      { type: 'h2', text: 'Software is never finished' },
      { type: 'p', text: 'A launch isn\'t a finish line. Cloud, DevOps and care runs from $4.5k a month, ongoing, and covers uptime monitoring with a named on-call person, monthly dependency and security patching, and, when needed, rescue work on a codebase someone else left in bad shape. We treat this as part of the product, not a line item sold after the fact. See pricing for how it fits with everything above.' },
      { type: 'h2', text: 'Can you start small' },
      { type: 'p', text: 'Yes. That\'s what discovery week is for. $4,800, low commitment, and it tells you what the real project looks like before either of us commits to more. We work remote-first. The spreadsheet doesn\'t care where you\'re located.' },
    ],
  },
  {
    slug: 'saas-development-agency-or-custom-web-app-studio-you-re-prob',
    title: 'SaaS Development Agency or Custom Web App Studio? You\'re Probably Asking the Wrong One',
    date: 'Sep 16, 2026',
    readingTime: '5 min read',
    category: 'Strategy',
    excerpt: 'SaaS development agency or custom web app studio — they\'re not the same purchase. A quick way to tell which one you need, plus real pricing, stack, and timelines either way.',
    body: [
      { type: 'p', text: 'Two people can type "SaaS development agency" into a search bar and want completely different things. One is trying to build a product to sell to strangers. The other is trying to fix the spreadsheet three people in their office are quietly terrified of. Most articles on this topic treat both as the same purchase with a comparison table in the middle. They\'re not the same decision. They don\'t cost the same, and they don\'t need the same vendor.' },
      { type: 'p', text: 'A SaaS development agency builds a product meant to be sold, by subscription, to outside customers who\'ve never met the founder. A custom web app is an internal tool built for one company\'s own team. Nobody outside that company will ever log in. If you\'re not sure which one you are, the rest of this is for you.' },
      { type: 'h2', text: 'What a SaaS development agency actually builds' },
      { type: 'p', text: 'A SaaS agency builds a multi-tenant product — one codebase serving many outside customers, each with their own account, their own data, walled off from each other. It needs self-serve signup, because a stranger with a credit card has to be able to sign up at 11pm without talking to anyone. It needs subscription billing: plans, upgrades, downgrades, failed payments. It needs onboarding built for people who\'ve never met the team that built the product.' },
      { type: 'p', text: 'That\'s a real specialty. The skills overlap with ours — React, Postgres, APIs — but the product decisions don\'t. None of that machinery shows up in a project built for one company\'s internal workflow, and building it in anyway is how you end up paying for features nobody at the company will ever touch.' },
      { type: 'h2', text: 'What we build instead' },
      { type: 'p', text: 'We build the other thing: a custom web app for one company\'s own team. Portals, dashboards, workflow tools — the software that runs the business, not the software the business sells. Role-based access, so the warehouse lead sees different screens than the CFO. Audit trails and approvals, because someone always eventually asks who changed this and when. Reporting built for finance teams who need numbers that tie out.' },
      { type: 'p', text: 'We migrate the data out of whatever system or spreadsheet is limping along now. We build the tool. We train the people who\'ll use it, with documentation that doesn\'t need translating before it\'s forwarded to someone else. See the custom web applications page for the specifics.' },
      { type: 'p', text: 'There\'s no billing layer in this work. There\'s no signup flow for strangers. There\'s one tenant — the company that hired us.' },
      { type: 'h2', text: 'If you\'re building something to sell, go find that shop' },
      { type: 'p', text: 'We turn down projects a specialist would do better. You\'ll hear that in the first call, not the third. We don\'t publish case studies for multi-tenant SaaS builds, because that isn\'t the work we take on.' },
      { type: 'h2', text: 'Four questions that tell you which one you actually are' },
      { type: 'p', text: 'Who is the end user — your internal staff, or external paying customers?' },
      { type: 'p', text: 'Is there a billing or subscription layer? Someone paying monthly to use the software is a different animal than a tool your own team uses because it\'s their job.' },
      { type: 'p', text: 'Is this one tenant or many — one company\'s data in one instance, or many companies\' data walled off inside the same product?' },
      { type: 'p', text: 'Is the goal to sell the software, or to fix a workflow problem inside one company?' },
      { type: 'p', text: 'Most people land clearly on one side once they actually answer these.' },
      { type: 'h2', text: 'What a real custom web app project looks like' },
      { type: 'p', text: 'Stack is React, Node, Postgres — boring, on purpose. Boring technology doesn\'t cost you a surprise migration three years after launch. Typical timeline is 10 to 20 weeks. Starting price is $45k. Migrating your existing data in, and training your team once it\'s live, is included, not billed as a surprise add-on at the end.' },
      { type: 'p', text: 'That\'s a defined scope with a number attached, not a range that runs from a weekend project to half a million dollars depending on who\'s asking.' },
      { type: 'h2', text: 'Discovery sorts out which one you are, before anyone commits' },
      { type: 'p', text: 'We run a discovery week before we quote anything. Flat $4,800, credited back in full if you continue with us. It ends in a written scope, an architecture sketch, and a fixed estimate. This is also where it becomes obvious if you\'re actually looking for a SaaS build dressed up as an internal tool — and we\'ll say so in week one, not month three.' },
      { type: 'p', text: 'Estimates come after discovery, not before. That\'s why the number holds.' },
      { type: 'h2', text: 'You own the code and the infrastructure, from day one' },
      { type: 'p', text: 'The repos live in your organisation, not ours. The infrastructure runs in your own cloud account, not a shared one we control. That matters more, not less, when it\'s your team\'s data and workflow on the line.' },
      { type: 'p', text: 'If the relationship ends, you get a full handover: an architecture walkthrough, runbooks, and two weeks of support. No cliff, no ransom.' },
      { type: 'h2', text: 'We don\'t take equity for a piece of your product' },
      { type: 'p', text: 'We don\'t take equity or offer deferred payment. It muddies the relationship, and it has never once made the software better. Cash, milestones, clear scope. If we have a stake in your company, we have an opinion about your roadmap that has nothing to do with whether the code is any good.' },
      { type: 'h2', text: 'Most people asking this question don\'t need a SaaS product at all' },
      { type: 'p', text: 'Most people typing "SaaS development agency" into a search bar aren\'t trying to build a product to sell to thousands of strangers. They\'re trying to kill the spreadsheet everyone secretly runs the business on. That\'s a build-vs-buy question for an internal team, whether or not the word "SaaS" made it into the search. Worth answering honestly before you talk to anyone — us included.' },
    ],
  },
  {
    slug: 'searching-for-a-saas-development-agency-you-might-want-the-o',
    title: 'Searching for a "SaaS Development Agency"? You Might Want the Other Kind of Shop',
    date: 'Sep 14, 2026',
    readingTime: '5 min read',
    category: 'Strategy',
    excerpt: '"SaaS development agency" usually means one of two very different projects. Here\'s how to tell which one you have, and what Codenest actually builds when it\'s the internal-tool kind.',
    body: [
      { type: 'p', text: 'Half the people typing "SaaS development agency" into a search bar are building a product they intend to sell. A multi-tenant thing meant for many outside customers, with subscription billing, a public marketing site, and a roadmap that gets pulled around by whatever the market wants next.' },
      { type: 'p', text: 'The other half are something else entirely. A mid-sized company that wants the spreadsheet everyone secretly runs the business on replaced with one proper internal tool. Same search term. Different project. Different shop. Different way of getting burned if you hire wrong.' },
      { type: 'p', text: 'This piece is for the second group.' },
      { type: 'h2', text: 'The one question that sorts you into the right pile' },
      { type: 'p', text: 'Is this software for strangers to sign up and pay for? Or is it for your own team, the people who already work for you, to run quoting, dispatch, approvals, or reporting?' },
      { type: 'p', text: 'If it\'s strangers, you\'re building a SaaS product. The person writing checks probably cares about self-serve onboarding, tiered pricing, and a roadmap shaped by a market you don\'t fully control yet. That\'s a specific discipline. Go find a shop that lives in it.' },
      { type: 'p', text: 'If it\'s just for you, the ops team, the dispatchers, the finance department reconciling numbers at midnight, you\'re not shopping for a SaaS agency at all. You\'re shopping for custom internal software. It looks similar from the outside. A login screen is a login screen. But the two are built and priced differently, and a shop that\'s good at one is often mediocre at the other.' },
      { type: 'h2', text: 'If you\'re building a product to sell, don\'t hire us' },
      { type: 'p', text: 'We turn down projects a specialist would do better, or that an existing off-the-shelf product already solves for a tenth of the price. You\'ll hear that in the first call, not the third.' },
      { type: 'p', text: 'Building a multi-tenant SaaS product, with billing and self-serve onboarding and everything that comes with selling software to strangers, is that kind of specialist work. If your project is headed for the open market, we\'d rather tell you that now than take the fee and hand you a generalist\'s version of a specialist\'s job.' },
      { type: 'h2', text: 'If it\'s the tool your team is stuck running in a spreadsheet, here\'s what that looks like' },
      { type: 'p', text: 'Usually it\'s a portal, a dashboard, or a workflow tool. The thing that replaces the spreadsheet with fifteen tabs and a macro nobody\'s allowed to touch.' },
      { type: 'p', text: 'It needs role-based access, because not everyone should see or approve everything. It needs an audit trail, because when a number changes, someone eventually asks who changed it. It needs reporting finance can actually use, migration of whatever data currently lives in the old system, and enough training and documentation that the tool survives the person who requested it going on vacation.' },
      { type: 'p', text: 'We build these on React, Node, and Postgres. Not because it\'s fashionable. Because it isn\'t. Boring technology, on purpose — novelty is a cost you keep paying every year after launch, not us.' },
      { type: 'p', text: 'A project like this typically runs 10 to 20 weeks and starts at $45k. That covers a straightforward approvals workflow on one end and a multi-department scheduling and reporting system on the other. Custom web applications has the full detail on what\'s in and out of scope.' },
      { type: 'h2', text: 'The messy part: some of what you have should stay SaaS' },
      { type: 'p', text: 'Most companies aren\'t choosing between "replace everything" and "keep everything." They\'re running a real mix. A SaaS accounting package here. A bought scheduling tool there. An old system nobody wants to touch because it still somehow works. Not all of it needs replacing. Some of it just needs to talk to the rest of it.' },
      { type: 'p', text: 'That\'s its own project, not a footnote to a bigger rebuild. Two-way sync with retry and reconciliation, so a failure doesn\'t quietly drop a record. Legacy systems handled even when they don\'t have an API to hook into. One warehouse where the numbers from every system land, so finance stops reconciling exports by hand. Failure alerts that name the specific record that broke, not a generic "sync failed" email nobody reads.' },
      { type: 'p', text: 'That kind of work usually runs 4 to 10 weeks, starting at $22k. Smaller, faster, and often the right first move before anyone commits to a full rebuild.' },
      { type: 'h2', text: 'How this gets priced before anyone signs anything' },
      { type: 'p', text: 'We don\'t guess at scope and hope the number holds. We run a discovery week first: a flat $4,800, credited back in full if you continue with us. It ends with three things in hand — a written scope, an architecture sketch, and a fixed estimate. Not a range. A number.' },
      { type: 'p', text: 'Estimates should come after discovery, not before. That\'s the whole reason ours hold. We\'d rather spend a week finding out what we\'re actually building, tell you what it costs, and let you decide with real information, including whether this is a custom-software project at all or something closer to the integration work above. Details are on the discovery week page.' },
      { type: 'h2', text: 'Why the lock-in worry probably sent you searching in the first place' },
      { type: 'p', text: 'If you\'ve been burned by software you don\'t control, a vendor who owns your data, a platform that changes pricing at renewal, a tool you can\'t leave without rebuilding from zero, that fear is probably doing more work in your search bar than the word "SaaS."' },
      { type: 'p', text: 'Here\'s where we stand on it. You own the code outright from day one. The repositories live in your organization, not ours. The infrastructure runs in your cloud account, not a black box we control. If we ever stop working together, that\'s how the arrangement was built from the start, not something you have to negotiate on the way out.' },
      { type: 'h2', text: 'You\'ll know in the first call' },
      { type: 'p', text: 'We\'d rather find out in twenty minutes that we\'re the wrong shop than find out four months into a project that never should have started. If it\'s the right fit, a named lead stays with you from that scoping call through launch. No handoff to whoever\'s free that sprint.' },
    ],
  },
  {
    slug: 'the-developer-is-gone-the-app-isn-t-now-what',
    title: 'The Developer Is Gone. The App Isn\'t. Now What?',
    date: 'Sep 9, 2026',
    readingTime: '7 min read',
    category: 'Strategy',
    excerpt: 'Your developer left and the app still needs to run. Here\'s how a real rescue engagement works: audit before quote, code ownership settled first, and a locked estimate.',
    body: [
      { type: 'p', text: 'The call usually comes about two weeks after it stopped being fine. The freelancer who built your dispatch tool stops answering emails, and everyone assumes he\'s just busy, until three weeks turn into three months. Or the developer who\'s been quietly maintaining your quoting system for four years hands in notice on a Friday, and nobody thought to ask where the deployment credentials live. Or the agency that built your customer portal shuts down entirely — website gone, phone disconnected, invoice from last quarter unanswered. Whatever the version, you end up in the same spot: the software is still running your business, and the only person who understood it is not available to ask.' },
      { type: 'p', text: 'Left no documentation. Left no handover notes. Left a repo you may or may not have access to, on infrastructure billed to a card nobody currently controls. The business runs on it anyway, because the alternative is going back to the spreadsheet everyone secretly ran the business on before this thing existed. This piece is for the company in that spot — not the solo founder patching a weekend project, not the office still running a database from thirty years ago. A real business, with a real system, and no one left who built it.' },
      { type: 'h2', text: 'Don\'t ask for a quote yet. Ask for an audit.' },
      { type: 'p', text: 'Anyone who gives you a number before reading the code is guessing, and you\'re paying for the guess either way — just later, and with a change order attached. You cannot price a fix for a system you haven\'t opened. You cannot promise a timeline for logic nobody has traced yet. The honest first step, when you\'re trying to take over an abandoned software project, is a paid, scoped audit with a defined output — not a free call that ends in a sales pitch.' },
      { type: 'p', text: 'That\'s what our discovery week is for. It\'s flat rate — $4,800 — and if you decide to continue with us, it\'s credited back in full. It\'s not a loss leader disguised as generosity; it\'s priced like the real work it is, because a code audit before hiring a developer takes real hours from someone senior enough to read the mess correctly. It ends in three things you can hold: a written scope, an architecture sketch, and a fixed estimate. Not a range. Not "depends on what we find" — because by the end of that week, we\'ve found it.' },
      { type: 'h2', text: 'Sometimes the honest answer is: don\'t patch it, replace it' },
      { type: 'p', text: 'We turn down projects a specialist would do better, or that an off-the-shelf product already solves for a tenth of the price. That applies here too, and it\'s the least popular thing we say in a first call, because it\'s the smaller job for us. But if the codebase fights the business at every turn — if every small change takes a week because nothing is where logic says it should be — patching it is the expensive option, not the cheap one. You\'re paying to preserve decisions nobody can explain anymore.' },
      { type: 'p', text: 'Part of why some rescues go badly has nothing to do with the age of the code and everything to do with what it was built on. We choose boring, established technology on purpose, because novelty is a cost paid every year after launch, by the client — and a system built on something clever and unmaintained is exactly how you end up making this same call again in two years. An honest audit tells you which situation you\'re in: a system worth saving, or one worth replacing on something that will still have documentation and hires available in five years.' },
      { type: 'h2', text: 'Before anyone touches the code, settle who owns it' },
      { type: 'p', text: 'This has to be a precondition, not a discussion that happens after work starts. You own the code outright from day one. The repos live in your organization, not a contractor\'s personal account. The infrastructure runs in your cloud account, not the vendor\'s — which matters enormously in a rescue, because half of these situations start with nobody being sure whose AWS bill it actually is.' },
      { type: 'p', text: 'We work under NDA by default, and we complete vendor security questionnaires as part of onboarding — not as a favor, as a normal step, the same way it would be for healthcare or fintech clients handling regulated data on a new build. On inherited codebase rescue work specifically, this ordering matters more than usual: you\'re granting access to a system you may not fully understand yet, to a vendor you haven\'t worked with before. Get the ownership terms settled before anyone gets a login, not after.' },
      { type: 'h2', text: 'Why a named lead matters more here than on a new build' },
      { type: 'p', text: 'On a new build, if your lead developer gets pulled onto something else mid-project, it\'s disruptive but recoverable — there\'s a spec, there\'s a plan, someone else can pick it up. On a rescue, there\'s no original developer left to explain why a decision was made five years ago. If the person who did the audit isn\'t the person doing the work, you\'ve re-created the exact problem you called about: knowledge that lives in one head and leaves when that head does.' },
      { type: 'p', text: 'That\'s why our named lead stays from the scoping call through launch. No handoff to a junior team partway through. On legacy system rescue work, the audit and the fix have to be done by the same person, because half the value of the audit is the judgment calls made while reading the code — and judgment doesn\'t transfer cleanly in a handoff memo.' },
      { type: 'h2', text: 'How the number gets locked, and stays locked' },
      { type: 'p', text: 'The architecture sketch, the written scope, and the fixed number all come out of discovery — that\'s the point of paying for it up front instead of skipping to a quote. Once we\'re past that, a fixed-scope project runs from $28,000 per phase, billed on milestones: typically 30% at kickoff, the rest on phase acceptance. Every fixed-scope engagement includes 30 days of post-launch fixes, so the number covers the shakeout period too, not just the day the thing ships. Billing terms are Net 14.' },
      { type: 'p', text: 'The prices we publish are starting points, not the final word — the discovery is what locks the actual number for your specific system. And if something comes up mid-project that wasn\'t in the original scope — a data migration that turns out messier than expected, an integration nobody mentioned in the first call — that gets priced before the work starts, not billed after the fact as a surprise. The number holds because it was never a guess to begin with.' },
      { type: 'h2', text: 'What happens after the rescue, so there isn\'t a repeat' },
      { type: 'p', text: 'A rescue that ends at launch just resets the clock. Software is never finished — ongoing care and improvement is part of the product, not an afterthought bolted on if you ask nicely. That\'s what Cloud, DevOps & Care is for: starting from $4.5k a month, cancel with 30 days\' notice, no long lock-in.' },
      { type: 'p', text: 'Concretely, that means infrastructure as code sitting in your own cloud account — so if this relationship ends too, you\'re not stranded the same way. It means uptime monitoring with a named on-call person, not a shared inbox. It means monthly dependency and security patching, so the system doesn\'t quietly rot the way the last one did. The whole point of this stage is that it\'s boring — nothing dramatic happens, which is exactly what you were missing the first time around.' },
      { type: 'h2', text: 'Questions to ask before you sign anyone up for this' },
      { type: 'p', text: 'Bring these to the first call, whoever you\'re talking to:' },
      { type: 'p', text: 'Who owns the code and the infrastructure when this ends? If the answer isn\'t "you, immediately," keep looking.' },
      { type: 'p', text: 'How do you bill? Vague answers here predict vague invoices later.' },
      { type: 'p', text: 'What happens if the estimate turns out to be wrong? There should be a mechanism for this, not a shrug.' },
      { type: 'p', text: 'Do you sign NDAs and security reviews? This should be routine, not a special request.' },
      { type: 'p', text: 'Do you take equity or offer deferred payment? We don\'t — it muddies the relationship and has never once made the software better. Cash, milestones, clear scope.' },
      { type: 'p', text: 'And ask what happens when it\'s over: a good vendor should offer a full handover with an architecture walkthrough, runbooks, and two weeks of support — the same thing you wish you\'d gotten from whoever left you here.' },
      { type: 'h2', text: 'Start with the discovery week, not the pitch' },
  ...
  {
    slug: 'you-searched-saas-development-agency-you-might-want-the-othe',
    title: 'You Searched "SaaS Development Agency." You Might Want the Other Thing.',
    date: 'Sep 7, 2026',
    readingTime: '5 min read',
    category: 'Strategy',
    excerpt: 'Most people searching "SaaS development agency" actually need an internal tool, not a sellable product. Here\'s how to tell the difference before you call anyone.',
    body: [
      { type: 'p', text: 'Two very different people type that phrase into Google, and almost nobody separates them. One is trying to build a company: a product with pricing tiers and a sign-up flow, meant to be sold to strangers who\'ve never heard of the business before. The other is trying to kill the spreadsheet everyone secretly runs the business on. Same search term. Completely different job. If you\'re the second person and you call a shop built for the first one, you\'ll get a pitch about tenancy models and go-to-market timing for a problem you don\'t have.' },
      { type: 'p', text: 'By the end of this you\'ll know which one you are. That matters more than which agency you call.' },
      { type: 'h2', text: 'What people usually mean by "SaaS development agency"' },
      { type: 'p', text: 'Strictly, SaaS means a multi-tenant product. One codebase, one database architecture, serving many outside customers who each sign up, pay, and get walled off from each other\'s data. The product is the business. Pricing tiers, churn, onboarding flows, a support team fielding tickets from customers you\'ve never met, all of it exists because the software itself is what generates revenue.' },
      { type: 'p', text: 'That\'s a real, specific kind of build. It can look, at a glance, like the same React-and-Postgres stack as an internal tool. It isn\'t. The assumptions underneath are different, and those assumptions get expensive to unwind later if a team gets them wrong early.' },
      { type: 'h2', text: 'What most mid-sized companies actually need' },
      { type: 'p', text: 'Most companies asking this question aren\'t building a company. They\'re running one, in logistics, healthcare, field services, manufacturing, and they\'ve outgrown the spreadsheets and off-the-shelf tools duct-taped together to keep it running. They need a portal for finance to close the month without three exports and a lookup formula. A dashboard their ops lead actually opens. A CRM or internal tool that handles quoting, dispatch, inventory and billing the way their business actually works, not the way a generic template assumes it works.' },
      { type: 'p', text: 'Nobody outside the company will ever log into this software. There\'s no pricing page for it, because it isn\'t being sold. It exists to get back the hour your team keeps losing to a system that was never built for them. This is the far more common case. Smaller in scope, no less serious about getting the details right.' },
      { type: 'h2', text: 'A quick way to tell which one you are' },
      { type: 'p', text: 'Will this be used by one company, yours, or sold to many customers who don\'t work here? Is there a pricing page and a tenant sign-up flow in your plan, or just your own team logging in with credentials IT already issued them?' },
      { type: 'p', text: 'If your answer to both is "just us," you\'re the second buyer. That\'s most people.' },
      { type: 'h2', text: 'If it\'s the first one, say so out loud' },
      { type: 'p', text: 'We turn down projects a specialist would do better. Multi-tenant SaaS is one of them. A firm that specializes in it will have opinions on billing at scale and tenant isolation that a generalist studio won\'t. You\'ll hear that from us in the first call, not the third, after a discovery week and an invoice.' },
      { type: 'p', text: 'That\'s not modesty. It\'s the same reason we don\'t take equity: it doesn\'t make the software better, and pretending otherwise wastes everyone\'s time, including yours.' },
      { type: 'h2', text: 'What we actually build instead' },
      { type: 'p', text: 'For the company running on its own tools, this is the actual work: custom web applications, portals, dashboards, workflow tools with role-based access and an audit trail finance can actually use. CRMs and internal tools covering pipeline, quoting, job scheduling, and invoicing that pushes straight into your accounting package instead of a spreadsheet someone re-keys every Friday. Built on React, Node, and Postgres. Built for companies that have outgrown what a template can do for them, not for companies trying to become the template.' },
      { type: 'h2', text: 'How it actually starts: a discovery week, not a guess' },
      ...
      { type: 'h2', text: 'The short version, before you make any calls' },
      { type: 'p', text: 'Will outside customers pay to use this, or will your own team log in to it? Is the real goal replacing a spreadsheet, or launching a product?' },
      { type: 'p', text: 'If it\'s the spreadsheet, that\'s us. If it\'s the product, a specialist in multi-tenant SaaS is worth calling before we are, and we\'ll tell you that ourselves, in the first conversation.' },
    ],
  },
  {
    slug: 'before-you-hire-a-saas-development-agency-ask-what-you-re-ac',
    title: 'Before You Hire a \'SaaS Development Agency,\' Ask What You\'re Actually Building',
    date: 'Sep 4, 2026',
    readingTime: '4 min read',
    category: 'Strategy',
    excerpt: 'Searching for a SaaS development agency? Most companies actually need custom internal software, not a resellable product. Here\'s the honest distinction, plus real pricing and timelines.',
    body: [
      { type: 'p', text: 'Two companies can search the exact same phrase and want opposite things. One is building a product to sell to hundreds of other companies. The other just wants the spreadsheet everyone secretly runs the business on to stop being a spreadsheet. Most of what ranks for "SaaS development agency" is written for the first company. Most people typing that phrase are the second.' },
      { type: 'h2', text: 'Two different projects, one search term' },
      { type: 'p', text: 'A multi-tenant product built to be sold needs billing, plans, tenant onboarding, an admin console for accounts you don\'t control, and support for strangers who signed up on a Tuesday. That\'s SaaS in the strict sense — software as a sold thing.' },
      { type: 'p', text: 'Then there\'s the other kind of project. One company\'s own tools. A dispatch board. A quoting tool. A dashboard finance actually trusts instead of double-checking against the version someone emails around. Nobody signs up for it from the outside. Nobody outside the company needs to be kept away from anyone else\'s data, because there is no anyone else.' },
      { type: 'h2', text: 'If it\'s the first one, we\'re probably not your studio' },
      { type: 'p', text: 'Building a sellable product with billing and tenant onboarding is its own pattern, with its own tradeoffs. It deserves a studio that works in that pattern every day, not one that treats it as a side case.' },
      { type: 'p', text: 'We turn down projects a specialist would do better. That\'s a conversation we\'ll have with you on the first call, not the third, and not after a deposit clears.' },
      { type: 'h2', text: 'What we actually build' },
      { type: 'p', text: 'Custom web applications for a company running its own operations. Role-based access, so a dispatcher and a controller see different things. Audit trails and approvals for the parts of the business that get audited. Reporting built for finance, not for a screenshot in a pitch deck. We migrate the data you already have out of spreadsheets and old systems instead of asking you to start from zero, and we write down how the thing works so your team isn\'t calling us every time someone new gets hired.' },
      { type: 'p', text: 'The stack is React, Node, and Postgres. Boring technology, on purpose — it\'s cheaper to run five years from now than whatever is exciting this year. Typical timeline runs 10 to 20 weeks, from $45,000. Not a floor we advertise and quietly abandon. A real number for a real kind of project, described in full here.' },
      { type: 'p', text: 'None of that needs billing plans or a signup flow for outside users. It needs to work for the people already on your payroll and survive contact with your actual data, which is always messier than the demo.' },
      { type: 'h2', text: 'Why the two get confused' },
      { type: 'p', text: 'Conflating the two inflates the estimate for something that never needed it. You end up paying for multi-tenant architecture and an admin console for accounts you will never have, because the framework you got quoted against assumed you were building a product to sell, not fixing the thing your ops team complains about every Monday. That\'s the difference between scoping your own tool and scoping a company you\'re not building.' },
      { type: 'h2', text: 'Discovery week settles it' },
      { type: 'p', text: 'We don\'t guess which category your project falls into on the sales call, and we don\'t want you guessing either. That\'s what discovery week is for: flat $4,800, credited back in full if you continue with us. It ends in a written scope, an architecture sketch, and a fixed estimate. Not a range. Not a "starting at."' },
      { type: 'p', text: 'Estimates come after discovery, not before. That\'s why the number holds. A studio that quotes off a first call is either padding the number to survive the unknowns, or getting it wrong and "adjusting" once work starts. Discovery also tells us, plainly, whether what you\'ve described is an internal tool with a fixed shape or something closer to a product that needs a different kind of specialist entirely.' },
      { type: 'h2', text: 'Turning down work is a filter, not a sales line' },
      { type: 'p', text: 'Saying no costs us money. A studio that never turns anything down doesn\'t have a filter, it has a sales process. We turn down projects a specialist would do better, or that an existing off-the-shelf product already solves for a tenth of the price. If your quoting problem is already solved by something sitting on a shelf, we\'ll tell you that instead of building a bespoke version of it at custom-software prices. You hear it in the first call. By the time a deposit clears, it isn\'t advice anymore, it\'s a sunk cost we let you make.' },
      { type: 'h2', text: 'You own the code either way' },
      { type: 'p', text: 'Whichever category your project turns out to be, the terms don\'t change. You own the code outright from day one. Infrastructure runs in your own cloud account, not ours, through cloud and DevOps work built on your infrastructure. There\'s nothing to hand back later because there was never anything for us to hold.' },
      { type: 'p', text: 'If what you need is a system that runs your own business better, that\'s the conversation to have with us. If what you need is a product to sell to other companies, we\'ll say so plainly instead of taking the work anyway.' },
    ],
  },
  {
